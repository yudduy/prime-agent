...
The runner checks the account's executable model list before work. Use `--model` to override an unavailable configured model for the pilot. `--preflight-dir` reuses a completed apparatus check only when the task source files and image digests match. Runtime failures remain unscored even when the final verifier can inspect a partial artifact.
