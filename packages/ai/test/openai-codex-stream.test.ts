...
describe("openai-codex streaming", () => {
	...
	it("preserves gpt-5.5 xhigh reasoning and omits the unsupported output-token field", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;
		const token = mockToken();
	...
	it("does not dispatch after a pre-request payload hook aborts", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;
		const token = mockToken();
	...
	it("honors a zero-retry transport policy", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;
		const token = mockToken();
		let providerRequestCount = 0;
