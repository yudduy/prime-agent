...
import { writeCompilerGymProxyCascadePreregistration } from "./compiler-gym-proxy-cascade-preregistration.js";
...
function parseCli(argv: readonly string[]): CliOptions {
	...
	if (!preregistrationPath) {
		throw new Error(
			"Usage: compiler-gym-proxy-cascade-preregistration-cli --output <path> [--output-dir <path>] [--repo-root <path>]",
		);
	}
	...
	console.log(
		canonicalJson(
			toJsonValue({
				...
				allocationCount: written.record.phasePlan.phaseCounts.total,
